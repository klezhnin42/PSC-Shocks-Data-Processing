...
%    return   
        ...
        y=h5read(address,'/y');
        z=h5read(address,'/z')/ sqrt(MMi/n)-97.1;
...
%        xs = h5read(address,'/xs')/ sqrt(MMi/n);
        zs = h5read(address,'/zs')/ sqrt(MMi/n)-97.1;
...
yedge = 0.25 * [-1:0.005:1]; %max(sqrt(px.^2+py.^2+pz.^2)) * [0:0.005:1];
...
%   text(37,0.13,'Shock layer','Rotation',70,'Color','white','FontSize',18)
   ...
   plot((1567/14.91-97.1).*ones(length(zindup)),zindup,'w--')
   plot((2089/14.91-97.1).*ones(length(zindup)),zindup,'w--')
   plot((2223/14.91-97.1).*ones(length(zindup)),zindup,'w--')

   text(3,0.15,'Piston','Rotation',90,'Color','white','FontSize',18)
   text(12,0.23,'Downstream','Color','white','FontSize',18)
   text(57,0.04,'Upstream','Color','white','FontSize',18)
   text(46,0.13,'Shock layer','Rotation',80,'Color','white','FontSize',18)
   text(-15,-0.13,'(b)')
%   text(0,0.04,'Upstream','Color','white','FontSize',18)

%    plot(mind/250*4.*ones(length(zindup),1),zindup,'r--');
% w   plwot((minddiff)/250*4.*ones(length(zindup),1),zindup,'g--');w
%    plot((mindup)/250*4.*ones(length(zindup),1),zindup,'b--');



    %title(sprintf('Simulation time, wci*t = %.3f', ts(k) *(dt * BB0/MMi)) )
%    title(strcat('Ion z-p_z phase plot',', \Omega_{i,up}t = 8.0'))%,num2str((ts+1000*k) *(dt * BB0/MMi),'%.1f')))

  saveas(h,strcat(datadir,'movie2.pngs/',particle,'zptot', num2str(ts(k),'%07d'),'_1.png'));
  delete(h);  
