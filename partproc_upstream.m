...
%    plot((mindup)/250*4.*ones(length(zindup),1),zindup,'b--');
   ...
   plot((1567/14.91).*ones(length(zindup)),zindup,'w--','LineWidth',2)
   ...
   legend({'\color{white} n_i','\color{white} B_y','\color{white} n_{i,upstream}'},'TextColor','white','Location','northwest')
...
%   title(sprintf('Simulation time, wci*t = %.3f', ts(k) *(dt * BB0/MMi)) )
  ...
  saveas(h,strcat(datadir,'movie2.pngs/',particle,'zptot', num2str(ts(k),'%07d'),'_upstream.png'));
  delete(h);  
