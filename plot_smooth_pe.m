...
%    size(loghgrm) s
    ...
    f=plot(8.0/200000.0*ts,smoothpe);
    hold on
    plot(Imax/length(ts)*8.0,Mmax,'ro')
    plot(Iefold/length(ts)*8.0,smoothpe(Iefold),'ro')
    (Imax-Iefold)/length(ts)*8.0
%    colorbar
%    caxis([1e-4 2])
%    set(gca,'YDir','normal')
...
%    shading flat
        ...
        view(2);
        saveas(gcf,strcat(datadir,'movie2.pngs/','pe_t',num2str(i),'.png'));
