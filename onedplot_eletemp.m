...
%  mean(TTzzi2(10500:13000))
    ...
    size(xs)
    meanTe=mean(Te,3);
    meanTex=mean(TTxxe,3);
    meanTey=mean(TTyye,3);
    meanTez=mean(TTzze,3);
  %  xs
...
%    zzz=plot(zs-107.5+10.4, squeeze(meanVze)/(V0),'cyan');
%    fff=plot(zs-107.5+10.4,squeeze(meanTe)/TTe/2,'g');
    fff1=plot(zs,squeeze(meanTex)/TTe);
    fff2=plot(zs,squeeze(meanTey)/TTe);
    fff3=plot(zs,squeeze(meanTez)/TTe);
...
%   asdf=plot(xind/(14.91)-97.1,mean(meanNNe(xind2))/n.*ones(length(xind2)),'r--')
%   scw=plot(xind/(14.91)-97.1,0.5*mean(meanTe(xind2))/TTe.*ones(length(xind2)),'g--')
%   dfgh=plot(xind/(14.91)-97.1,0.1*mean(meanTi(xind2))/TTe.*ones(length(xind2)),'m--')
   
%   plot((1567/14.91-97.1).*ones(length(zindup)),zindup,'k--')
%   plot((2089/14.91-97.1).*ones(length(zindup)),zindup,'k--')
%   plot((2223/14.91-97.1).*ones(length(zindup)),zindup,'k--')

%   text(3,20,'Piston','Rotation',90)
%   text(10,15,'Downstream')
%   text(60,4,'Upstream')
%   text(47 ,17,'Shock layer','Rotation',80)
%   text(-10,-5,'(a)')
...
%    
    ...
    legend({'B_y/B_{up}','n/n_{up}','T_{i,x}/T_{up}','T_{i,y}/T_{up}','T_{i,z}/T_{up}'},'FontSize',10,'Location','northeast')
...
%  title(strcat('Ablation',', \Omega_{i,up}t =',num2str((ts+1000*k) *(dt * BB0/MMi),'%.1f')))
    view(2);
    saveas(gcf,strcat(datadir,'1d_eletemp_', num2str(ts+1000*k,'%07d'),'_4.png'));
%  end 
