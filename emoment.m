...
%    size(en)
%    size(hgrm)
%    plot(en,hgrm./transpose(en)/maxvalspec*TTe,'LineWidth',3)
%    hold on
%    if(k==4)
...
%    en = logspace(-6.0,1.1,200);
    ...
    ennth = linspace(EnMax*0.02,EnMax,1000);
    hgrmnnth = histc(ElectronEnergyUp, ennth);

    sum((hgrmnnth).*transpose(ennth))/sum((hgrmlin).*transpose(enlin))   
    sum((hgrmlin).*transpose(enlin))/sum(hgrmlin)
...
%plot(en,(0.2*(en/(1.0*TTe*11.3676568)).^0.5.*exp(-en/(1.0*TTe*11.3676568))+0.0*(en/(30*TTe*11.3676568)).^0.5.*exp(-en/(30*TTe*11.3676568))),'ro')
...
%legend5 = 'Fit, t=8 \Omega_i^{-1}';
%legend4 = 'Fit, t=8 \Omega_i^{-1}';% 1.5* 14 eV + 5e-3* 889 eV');
%legend2 = sprintf('2 T_0');
%legend3 = sprintf('3 T_0');
%legend4 = sprintf('4 T_0');
%legend({legend4,legend0,legend1},'Location','south');
...
%set(gca, 'XScale', 'log');
%set(gca, 'YScale', 'log');
%saveas(h,strcat(datadir,particle,'EnergySpectrumUpstream2Evol','.png'),'png');
%delete(h);
