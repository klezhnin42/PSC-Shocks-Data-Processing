...
%    ElectronEnergyFoot(indices3) = [];
    ...
    indices4 = find(z<xshock(k) | z>xdown(k));
    ...
    indices6 = find(z<xshock(k) | z>xdown(k));
...
plot(en,maxvalspec*exp(-en/(TTe*25*11.3676568))+1e-2*maxvalspec*exp(-en/(TTe*50*11.3676568)),'r--','LineWidth',3)
hold on
...
text(0.4,3e5,'T_{e,bulk} \sim 568 eV','FontSize',24,'Rotation',-40)
text(2,8e2,'T_{e,nth} \sim 1137 eV','FontSize',24,'Rotation',-35)
text(-1.5,0.1,'(c)','FontSize',24)
%set(gca,'FontSize',20)
%title('Shock layer electron energy spectrum','FontSize',16)
...
ylim([1 5e6]);
   %set(gca, 'XScale', 'log');
set(gca, 'YScale', 'log');
saveas(h,strcat(datadir,particle,'EnergySpectrumShockEvol','.png'),'png');
delete(h);
