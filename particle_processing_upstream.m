...
plot(en,maxvalspec*exp(-en/(TTe*1.8*11.3676568))+2e-4*maxvalspec*exp(-en/(TTe*40*11.3676568)),'r--','LineWidth',3)
hold on
...
text(0.6,4e6,'T_{e,bulk} \sim 45 eV','FontSize',24,'Rotation',-75)
text(1,2e2,'T_{e,nth} \sim 909 eV','FontSize',24,'Rotation',-35)
text(-1.5,0.1,'(d)','FontSize',24)
%et(gca,'FontSize',20)
%title('Upstream electron energy spectrum','FontSize',16)
...
ylim([1 5e6]);
   %set(gca, 'XScale', 'log');
set(gca, 'YScale', 'log');
saveas(h,strcat(datadir,particle,'EnergySpectrumUpstreamEvol','.png'),'png');
delete(h);
