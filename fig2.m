...
%   text(46+98.4,0.8,'Shock layer','Rotation',90,'Color','black','FontSize',19)
   text(-25,-0.5,'(a)','FontSize',14)
   legend({'\color{black} n_i','\color{black} B_y','\color{black} n_{i,piston}'},'TextColor','black','Location','northeast','FontSize',8)
...
%   plot(zedges,5.45*zvals/max(zvals),'black','LineWidth',1.5);
   plot(zedges,0.6*zvals/max(zvals),'black','LineWidth',1.5);
%    plot(mind/250*4.*ones(length(zindup),1),zindup,'r--');
%    plwot((minddiff)/250*4.*ones(length(zindup),1),zindup,'g--');w
%    plot((mindup)/250*4.*ones(length(zindup),1),zindup,'b--');
   zindup=[-50:49];
   ...
   text(-25,-0.5,'(b)','FontSize',14)
%  text(-25,-3.5,'(c)','FontSize',14)
   legend({'\color{black} n_i','\color{black} B_y','\color{black} n_{i,upstream}'},'TextColor','black','Location','northeast','FontSize',8)
...
%xsdf=plot(en,(1.0*(en/(1.5*TTe*11.3676568)).^1.5.*exp(-en/(1.5*TTe*11.3676568))+0.0*(en/(30*TTe*11.3676568)).^0.5.*exp(-en/(30*TTe*11.3676568))),'g--')
...
%legend({legend4,legend0,legend1,legend2,legend3,legend5},'Location','eastoutside','FontSize',8);
...
%    set(h, 'PaperSize', [4 8]);
  ...
  saveas(h,strcat(datadir,'Fig2.png'));
  delete(h);
