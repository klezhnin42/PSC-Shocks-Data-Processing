...
%  mean(TTzzi2(10500:13000))
    ...
    size(xs)
    meanTe=mean(Te,3);
    meanTix=mean(TTxxi,3);
    meanTiy=mean(TTyyi,3);
    meanTiz=mean(TTzzi,3);
  %  xs
...
%    fff=plot(zs-107.5+10.4,squeeze(meanTe)/TTe/2,'g');
    fff1=plot(zs,squeeze(meanTix)/TTe);
    fff2=plot(zs,squeeze(meanTiy)/TTe);
    fff3=plot(zs,squeeze(meanTiz)/TTe);
...
%  title(strcat('Ablation',', \Omega_{i,up}t =',num2str((ts+1000*k) *(dt * BB0/MMi),'%.1f')))
    view(2);
    saveas(gcf,strcat(datadir,'1d_iontemp_', num2str(ts+1000*k,'%07d'),'_4.png'));
%  end 
