...
%    hold on
 % size(squeeze(bx))w5
    xlim([0 200]);
    xlabel('z/d_{i,up}');
    ylabel('p_{e}/m_ec, 0.1n_i/n_{up}, 0.1B_y/B_{up}')
...
%  plot(zedges,5.5*zvals/max(zvals),'green','LineWidth',1);
...
%    plot((mindup)/250*4.*ones(length(zindup),1),zindup,'b--');
   ...
   plot(153.*ones(length(zindup)),zindup,'k--','LineWidth',2)
   plot(zindup*4+200,0.3.*ones(length(zindup)),'k--','LineWidth',2)
%   plot((2089/14.91).*ones(length(zindup)),zindup,'w--','LineWidth',2)
%   plot((2223/14.91).*ones(length(zindup)),zindup,'w--','LineWidth',2)

%   text(30,0.1,'Piston','Rotation',0,'Color','white','FontSize',19)
%   text(14+97.1,0.8,'Downstream','Rotation',70,'Color','white','FontSize',19)
%   text(53+97.1,-0.1,'Upstream','Color','white','FontSize',19)
%   text(46+98.4,0.8,'Shock layer','Rotation',90,'Color','white','FontSize',19)
%   text(-25,-0.5,'(b)','FontSize',24)
%   legend({'\color{white} n_i','\color{white} B_y','\color{white} n_{i,piston}'},'TextColor','white','Location','northwest')
...
%   title(sprintf('Simulation time, wci*t = %.3f', ts(k) *(dt * BB0/MMi)) )
  ...
  saveas(h,strcat(datadir,'movie2.pngs/',particle,'zptot', num2str(ts(k),'%07d'),'_up.png'));
  delete(h);  
