...
rfrmtn=load('reformation.mat','rfrmtn');
...
%    ylim([0 5])
    xlim([0 220.0])
    title('log_{10} n_e(z,t)/n_{up}'); %sprintf('wci*t = %.3f', (ts+1000*k) *(dt * BB0/MMi)) )

    view(2);
    saveas(gcf,strcat(datadir,'tracks/','Reformation',num2str(idLfast(i)),'.png'));
