...
%    meanTi=mean(Ti,1);
  %  xs
    ...
    meanNNe=log10(mean(efield,3)/BB0);
...
%    ylim([0 5])
    xlim([0.0 220.0])
    title('Evolution of log10(E/E_0) averaged along x'); %sprintf('wci*t = %.3f', (ts+1000*k) *(dt * BB0/MMi)) )

    view(2);
    saveas(gcf,strcat(datadir,'Reformation', '.png'));
