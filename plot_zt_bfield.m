...
rfrmtn=load('byreformation.mat','rfrmtn');
...
plot(xfast.xfast,times2.times2,'Color','black')
...
set(gca,'YDir','normal')
    ...
    ylim([4 7.95])
    xlim([100 200.0])
    title('B_y/B_{0}'); %sprintf('wci*t = %.3f', (ts+1000*k) *(dt * BB0/MMi)) )

    view(2);
    saveas(gcf,strcat(datadir,'zt_byfield','.png'));
