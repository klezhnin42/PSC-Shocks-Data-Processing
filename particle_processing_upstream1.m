...
xshock = [32.0, 67.0, 104.0, 140.0];
xdown = [42.0, 77.0, 114.0, 150.0];
...
%    size(en1)
%    size(hgrm)
    plot(en,hgrm/maxvalspec,'LineWidth',3)
    hold on
    if(k==4)
    	hgrmpiston=histc(ElectronEnergyPiston, en)/maxvalspec;
%,'--','LineWidth',3,'Color',[0.4940, 0.1840, 0.5560])
    end
...
plot(en,exp(-en/(TTe*1.8*11.3676568))+1.3e-2*exp(-en/(TTe*30*11.3676568)),'r--','LineWidth',3)
hold on

legend0 = 't=2 \Omega_i^{-1}';
legend1 = 't=4 \Omega_i^{-1}';
legend2 = 't=6 \Omega_i^{-1}';
legend3 = 't=8 \Omega_i^{-1}';
% egend4 = 't=8 \Omega_i^{-1}, piston particles';
...
plot(en, hgrmpiston,'--','LineWidth',3,'Color',[0.4940, 0.1840, 0.5560],'HandleVisibility','off')

text(0.6,1e0,'T_{e,bulk} \sim 45 eV','FontSize',24,'Rotation',-60)
text(1,3e-4,'T_{e,nth} \sim 682 eV','FontSize',24,'Rotation',-35)
text(-1.5,1e-8,'(c)','FontSize',24)
%et(gca,'FontSize',20)
%title('Upstream+shock e spectrum','FontSize',16)
...
ylim([1e-7 2e0]);
   %set(gca, 'XScale', 'log');
set(gca, 'YScale', 'log');
saveas(h,strcat(datadir,particle,'EnergySpectrumUpstream1Evol2','.png'),'png');
delete(h);
