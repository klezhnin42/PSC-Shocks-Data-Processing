...
%    zsdf=plot(xs, squeeze(meanby)/BB0);
    address=strcat(datadir, 'ion','_',num2str(ts(k),'%07d'),'_0.h5')
 %   try
...
%    xlabel('z/d_{i,up}');
   xlabel('z / d_{i,up}')
   gggg=plot(zmax.*ones(length(zindup)),zindup,'k--')
   ...
   view(2);
   saveas(gcf,strcat(datadir,'ionzpz_', num2str(ts(k),'%07d'),'_6.png'));
%  end 
