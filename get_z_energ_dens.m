...
for p=1:length(ts)
        ...
        indices4 = find(petot>0.3);
        pefaste=petot(indices4);
        fraction_energ=[fraction_energ; size(pefaste,1)/size(petot,1)]
...
%    hold on
    ...
    [zvals,zedges]=histcounts(z,xedge);
    size(zvals)
    max(zvals)
    xedge(1)=[];
    zzz=plot(xedge,zvals/max(zvals),'blue');
...
%        ylabel('\Omega_i t','FontSize',20)
        ...
        plot(ts*(dt * BB0/MMi),fraction_energ*100)
        ...
        view(2);
        saveas(gcf,strcat(datadir,'energ_evol','.png'));
