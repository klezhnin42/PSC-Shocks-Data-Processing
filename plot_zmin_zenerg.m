...
for i=1:size(z,1)
    ...
    ind=min(indices);
    zenergarr=[zenergarr; z1sh(ind)];
    zminarr=[zminarr; min(z1sh)];
end
...
%xedge = max(sqrt(px.^2)) * [-1:0.005:1];
    ...
    sum(zenergarr)/length(zenergarr)
    sum(zminarr)/length(zminarr)
    hgrm=histc2d(zenergarr,zminarr,xedge,yedge);
    loghgrm=log10(hgrm);
...
%   loghgrm(~isfinite(loghgrm))=0;
    ...
    hold on
    plot(linspace(-50,50),linspace(-50,50),'r--')
    plot(linspace(-50,50),linspace(0,0,100),'k--','LineWidth',0.5)
    colorbar
    caxis([0 2])
    set(gca,'YDir','normal')
    ...
    shading flat
        ...
        xlabel('Z_{energ}-Z_{shock}')
        ylabel('Z_{min}-Z_{shock}')
        ylim([-25 25])
        xlim([-25 25])
        %set(gca, 'YScale', 'log')
        ...
        view(2);
        saveas(gcf,strcat(datadir,'zenerg_zmin_hist2d','.png'));
