...
for k=0:sizze
  ...
  Ti (Ti < 0) = 1e-6;
    ...
    size(xs)
    meanTe=mean(Te,1);
    meanTTxxi=mean(TTxxi,1);
    meanTTyyi=mean(TTyyi,1);
    meanTTzzi=mean(TTzzi,1);
  %  xs
...
%    hold on
    ...
    xxx=plot(xs, log10(squeeze(meanNNe)/n));%
    hold on
%    zzz=plot(xs, squeeze(meanVxe)/(V0));
%    fff=plot(xs,squeeze(meanTe)/TTe);
    yyy=plot(xs,log10(squeeze(meanTTxxi)/TTe));
    zzz=plot(xs,log10(squeeze(meanTTyyi)/TTe));
    fff=plot(xs,log10(squeeze(meanTTzzi)/TTe));
%    meanTe=mean(Te,1);
...
%    
    ...
    legend({'n','T_{xi}', 'T_{yi}', 'T_{zi}'},'FontSize',10,'Location','southeast')
...
%    ylabel('{B_x [3.0 B0], n_e [0.6n_{\rm max}], v_{ez} [0.42 C_{s}] }','FontSize',20)
    ...
    view(2);
    saveas(gcf,strcat(datadir,'TempIon1d_', num2str(ts+1000*k,'%07d'),'_2.png'));
