...
datadir = './h5/'; % '/Users/klezhnin/Desktop/shock/largemmi/1d_MMI_400/theta60/';
...
%  delete(h);   
    ...
    ElectronEnergy=((px).^2+py.^2+pz.^2)./2;
...
%    PhotonEnergy(indices) = [];
   ...
   % meandensdownarray=meandensdown.*ones(length(xind3),1);
    ...
    EnMax=1.0; %(max(ElectronEnergyDown)); %
...
%    plot(en,hgrm2)
    ...
    plot(en,1.5*maxvalspec*exp(-en/(4*TTe))+7e-4*maxvalspec*exp(-en/(TTe*80)),'r--')
...
%    plot(en,maxvalspec*exp(-en/(TTe*4)),'c--')
    ...
    legend1 = sprintf('1.5*4T_0 + 7e-4* 80T_0');
